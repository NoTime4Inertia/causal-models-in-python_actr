...
# disadvantages (1) yield time is always the same (2) cannot use for parallel actions
    ...
    def watchBomb(self, cue):
        self.parent.parent.wires(self.parent.parent.red_wire, self.parent.parent.blue_wire) # set to match declarative inference
        if cue == 'X':
...
class MyAgent(ACTR):
    ...
    DM.add('metainf:disarm object:bomb interv2:blue_wire_UT isthen:disarmed') # red wire cut second
