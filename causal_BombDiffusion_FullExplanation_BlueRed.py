...
## this is a model that shows causal inference using meta-inferential knowledge built out of
## counterfactuals, interventions, and associations.
## Additionally, Kantian Declarative Memory provides the base faculties required to define each
## value with a relevant index, which serves as a faculty to specify how it is associated with the rest
## of declarative memory.

## The model itself involves a bomb technician (agent) who must cut red and blue wires in a
## specific oder for different parts of the bomb, to disarm it. If it does not use that order,
## the bomb remains armed. The agent thinks about which plan is best,
## enacts the plan, and then suggests what would leave the bomb armed.

## The task has a single solution, with only two outcomes (armed and disarmed).
...
## Change self.blue_wire and self.red_wire to alter disarmament protocol
    ...
    def wires(self, wire1, wire2):
        if wire1 == self.blue_wire and wire1.state == 'cut':
            if wire2 == self.red_wire and wire2.state == 'cut':
...
# disadvantages (1) yield time is always the same (2) cannot use for parallel actions
    ...
    def watchBomb(self, cue):
        self.parent.parent.wires(self.parent.parent.blue_wire, self.parent.parent.red_wire) # exec disarm before watching.. not ideal but it'll do
        if cue == 'X':
...
class MyAgent(ACTR):
    ...
    # KDMs suck. What a nightmare. Munchhausen lives here.
    # Indexes are basically kantian concepts that pass info through them in structured ways to reason about the values
    # Any architecture without an index has index in specified structural features that pass information (reasoning) upstream
    # Munch, halt, and undecide go in DISCUSSION!!!
    ...
    # Object Knowledge
    ##Note: Old act-r did a lot of self-referential isa's
    ...
    # Intervention Knowledge of Action on Objects at Unit Task Level
    ...
    # Intervention Knowledge of Action on Objects at Planning Unit Level
    ...
    ## Knowledge of outcome from changes of object states based on actions taken

    # What does each SGOMS component do? All levels can be inferred from PU counterfact outcomes (isthen)
       
    #DM.add('metainf:disarm object:bomb interv:X isthen:disarmed')

    #DM.add('metainf:disarm object:bomb interv1:blue_wire_UT isthen:disarmed') # blue wire cut first
    
    DM.add('metainf:disarm object:bomb interv2:red_wire_UT isthen:disarmed') # red wire cut second
...
## Use this to set the correct combination for disarming the bomb
